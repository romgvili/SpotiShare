...
def main():
    ...
    def addsongpressed():
        ...
        url = urlopen(artist['images'][0]['url']).read()
        url = io.BytesIO(url)
        artistpic =pg.image.load(url)
        ...
        for item in albums:
            ...
            for item in trackResults:
                if songname in item['name'].upper():
                    clientSocket.send(item['uri'].encode())
                    print(clientSocket.recv(55).decode())
                    screen.blit(yo, (0, 0))
                    return
                if levenshtein(songname,item['name'].upper())<4.0:
                    close[item['uri']] = levenshtein(songname,item['name'].upper())<4.0
        if len(close)!=0:
            print(close)
            closest = ""
            mini = 5
